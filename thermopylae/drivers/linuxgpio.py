...
class LinuxGPIO:
    ...
    # precision: how many digits to the right of the decimal place is the thermometer capable of
    def __init__(self, target_thermometer, precision):
        self.target_thermometer = target_thermometer
        self.divisor = 10 ** precision

    def read(self):
        thermometer_file = os.path.join('/', 'sys', 'bus', 'w1', 'devices', self.target_thermometer, 'w1_slave')
        if not os.path.exists(thermometer_file):
            return None
        ...
        temp = float(temp_raw) / self.divisor
        return temp
